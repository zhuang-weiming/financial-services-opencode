...
def evaluate_one(code, n_eval=52, step=5):
    ...
    v3.2 修正 (用户 8-11 反馈):
      ...
      - 新版: 5 天 1 点 × 52 点 = 260 天 (周线评估节奏, 1 年中期视角)
      - 对照: 实盘盘后看周线信号, 1 周 1 次信号更新
    ...
    if len(eval_idxs) < 12:  # 周线评估至少需 12 个点 (约 3 个月信号)
        return None
    ...
    for i, code in enumerate(universe):
        if i % 100 == 0:
            ...
            print(f"  [{i}/{len(universe)}] ok={len(results)} fail={fail_count} "
                  f"elapsed={elapsed:.0f}s rate={rate:.2f}/s eta={eta:.0f}s", flush=True)
        try:
