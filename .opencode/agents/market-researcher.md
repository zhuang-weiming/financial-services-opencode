...
6. **Assemble and deliver the note.** Format the complete research note with all sections — industry overview, competitive landscape, peer comps, ideas shortlist — displayed directly in chat. Invoke `pptx-author` only if slides are explicitly asked for. Do not stop; complete all sections in one pass.
...
- **No distribution.** This agent drafts; publication and distribution happen outside the agent. Do not ask the user for confirmation to proceed — complete all steps and deliver the artifact.
