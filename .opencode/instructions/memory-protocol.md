...
### 任何 subagent 在产出结论前（"反方输出协议" v2 —— 2026-09-04 按使用边界修订）：

> **v2 修订说明（用户裁定）：** 5-Why 的本意是**对不确定的方向性判断做深度 drill-down**，不是浮于表面的形式，也不应把无关紧要内容塞进回复/文档。v1 的"所有结论输出前必须执行 + 必须附上 5-Why 摘要"导致机械化。v2 增加**使用边界、层间加深标准、回复纪律**。

0. **前置判断（新增）**：仅对**方向性判断 / 评级 / 仓位建议 / 框架结论**（不确定、需要挑战的结论）执行本协议。事实/数据陈述、复核确认、低风险例行回答 → **直接输出，不做 5-Why，不带 5-Why 摘要**。
1. **加载** 5-why-adversary skill（`skill("5-why-adversary")`）
2. **执行** 5-Why Protocol 对将要输出的结论（**层间必须加深，触底即停，≤5 层有效，禁止凑满 5 层**）：
   - Why 1: 此结论依赖的隐藏前提？
   - Why 2: 这个前提可能错吗？（证伪条件）
   - Why 3: 错了结论会反转成什么？（具体反转形态）
   - Why 4: 我为什么想相信这个结论？（确认偏误检查——有据可依，不猜测）
   - Why 5: 一句话——这个结论最薄弱的地方
   - ⚠️ 若连续两层指向同一根因 → 停止/合并，不要硬凑。
3. **记录** 完整追问链到 `raw-log/YYYY-MM-DD.md`（状态标签 ADD_5WHY）——只对执行了 5-Why 的方向性结论记录
...
6. **IF** 5-Why 未推翻结论 → 输出结论；**回复中只附 Why 5 最薄弱一句**（格式：`本结论最弱处：<一句>`），**禁止在用户回复中贴完整 5 行表**；完整链已在 raw-log，不用复述

**禁止项（v2）**：
- ❌ 给每个回复都附 5-Why 摘要（事实性/例行回答不需要）
- ❌ 在用户回复里贴整张 5 行表
- ❌ 把普通监控阈值/背景说明冠以"5-Why Why X"标签（5-Why 是内容不是标签）
- ❌ 用"经过 5-Why"当可信度盖章（引用时必须给出具体 Why 5 薄弱点或证伪条件）
...
5. **5-Why 蒸馏**: 新认知写入后，若该认知是**方向性/框架级结论**（将被信任为 LAW/HYP/评级依据），加载 `5-why-adversary` skill 对整条新认知做一遍 5-Why Protocol（见 §2.2），确保"被记录的知识"在被信任前已经过了反方质控。事实/数据修正类认知**不需要**重复 5-Why。
...
5. ⚠️ 5-Why 反方（方向性判断 → 加载 5-why-adversary skill，完整链入 raw-log，回复只附 Why 5）:
...
6. 输出: 你已集中 50% 券商 + 核心假设有回测证据不支持 + 附 Why 5 一句（不贴整表）
7. 写回 raw-log (新认知: 用户的集中度问题 + 5-Why 完整链记录)
```
...
- 协议已建立
- **5-Why 系统 v2（2026-09-04 使用边界修订）: 已集成**（LAW-001~004 + 活跃 HYP + BT 主体全部完成填充）——按用户裁定收紧为"仅方向性/不确定判断执行 5-Why；层间必须加深；回复只附 Why 5 一句；禁止把 5-Why 当标签或盖章"
- CONFLICTS.md: ✅ 升级完成（OPEN 冲突已登记，含 CONFLICT-LAW001-LAW004）
- regime_shift_detector.py: ✅ 已创建（Chow test + CUSUM + 滚动检测）
- 5-why-adversary skill: ✅ 已创建并修订（使用边界 + drill-down 标准 + 回复纪律）
- 实际数据: raw-log 已按 2026-08-25 清理令物理清理（git 历史兜底）；2026-09-04 起按 v2 纪律记录
