...
- `get_sec_filings` is an agent + CLI tool. For deeper interpretation (reading MD&A, scoring insider/8-K signals), pair it with the `vibe-trading-edgar-sec-filings` methodology skill, which delegates here for fetching.
