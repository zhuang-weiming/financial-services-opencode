---
name: vibe-trading-ashare-pre-st-filter
...
> **依赖**：本 skill 必须配合 [tushare skill](../vibe-trading-tushare/SKILL.md) 使用，缺数据时回退到 [akshare skill](../vibe-trading-akshare/SKILL.md)。
...
1. **所有财务/基本面/分红/审计/ST 状态数据，必须先调用 [tushare skill](../vibe-trading-tushare/SKILL.md)**，按下表指定的接口名、`ts_code`、`period` 拉取。
2. 当 tushare 接口因积分不足、token 缺失或返回空时，**回退到 [akshare skill](../vibe-trading-akshare/SKILL.md)** 对应函数。
