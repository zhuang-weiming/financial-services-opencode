---
name: vibe-trading-strategy-generate
...
Synced to **Vibe-Trading v0.1.15** (2026-09-09). Three new first-class markets
(UK LSE, Korea KRX, Vietnam HOSE) and one explicit-only hosted source
(TickerAll MT5) are listed below.

| Pattern | Market | source | Notes |
|------|------|--------|----------|
...
| `^[A-Z]+\.US$` | US stocks | yfinance | dotted class shares (e.g. `BRK.B.US`) supported in v0.1.15 |
| `^\d{3,5}\.HK$` | Hong Kong stocks | yfinance / longbridge | |
| `^[A-Z]+\.TO$` / `^[A-Z]+\.V$` | Canada (TSX / TSXV) | yfinance | CAD identity, whole shares |
| `^[A-Z0-9]+\.NS$` / `^[A-Z0-9]+\.BO$` | India (NSE / BSE) | yfinance | T+1, circuit bands, STT cost; live trading structurally disabled |
| `^\d{6}\.KS$` / `^\d{6}\.KQ$` | Korea (KOSPI / KOSDAQ) | pykrx | **New in v0.1.15.** Same-day round-trip (no T+1), ±30% band, 0.20% sell-side transaction tax, **long-only**. |
| `^[A-Z]+\.VN$` | Vietnam (HOSE) | yfinance | **New in v0.1.15.** Long-only, ±7% band, 100-share lots, 0.1% sell-side personal income tax. HNX/UPCOM unsupported. |
| `^[A-Z]+\.L$` / `^[A-Z]+\.IL$` | UK (LSE) | yfinance | **New in v0.1.15.** SDRT 0.5% purchase-side duty (not symmetric). |
| `^[A-Z]+-USDT$` | Cryptocurrency | okx | |
| `^\d{6,8}\.(SHFE\|CFFEX\|GFEX\|ZCE)$` or `^[A-Z]{1,2}\d{1,4}$` | China Futures | akshare (Sina daily endpoints) | **New in v0.1.15.** Main continuous contracts (`RB0`, `IF0`) route correctly. |
| `^[A-Z]+Z\d{1,2}\.[A-Z]+$` (e.g. `CL2412.NYMEX`, `ESZ4.CME`) | Global Futures | yfinance / akshare / mt5 / tickerall | Dated contracts carry venue (v0.1.15 fix). |
| `^[A-Z]{3}/[A-Z]{3}$` | Forex | yfinance / alphavantage / mt5 / tickerall | TickerAll = hosted MT5 HTTP, **explicit-only**. |
...
**Data window separation (v0.1.15)**: warm-up bars stop counting as
evaluation. A strategy is no longer graded on the bars it needed to
become defined. The skill does not expose this as a config knob — it is
enforced inside the backtest engine, but you should know about it when
you compare a new backtest's metrics against an older run.

**Adjustment caliber**: every served price frame is stamped with its
caliber (`split_adjusted` / `dividend_adjusted` / `raw` / `unknown`).
When you cite a price next to a backtest output, mention which caliber
the runner used. Anything unmeasured resolves to `unknown` on purpose —
for example, Yahoo serves split-adjusted quotes with no adjustment logic
in this repo.
...
- See the [cross-market-strategy](../vibe-trading-cross-market-strategy/SKILL.md) skill for per-market parameters, vol-adjustment, and example code
