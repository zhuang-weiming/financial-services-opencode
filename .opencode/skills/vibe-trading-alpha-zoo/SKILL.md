...
> **v0.1.15 upgrade (2026-09-09)** — zoo now bundles **462 alphas** across the
> 5 families (was 461). The NaN contract is enforced at the registry: an alpha
> that branches on `np.where` over a missing input is masked to NaN at the
> output instead of returning a fabricated value. The `pct_change()`
> forward-fill class is closed repo-wide, so a missing close no longer reads
> as a `0.0%` return.
...
| `gtja191` | Guotai Junan 191 alphas — A-share focused cross-sectional factors. Tagged for `equity_cn`. | ~191 |
| `qlib158` | Microsoft Qlib's 158 alpha factors — features tuned for ML pipelines. Also tagged for `equity_in` (India NSE/BSE) and `equity_kr` (KRX). | ~158 |
| `alpha101` | WorldQuant 101 formulaic alphas (subset used by qlib). | ~101 |
| `classical` | Fama-French 3/5-factor + Carhart momentum. | <10 |
| (sum) | **Total alphas currently registered** | **462** |
...
- **NaN is preserved through warm-up and missing data** (v0.1.15 NaN contract,
  enforced at `Registry.compute`). No silent `fillna(0)`. An alpha whose
  declared input is missing on a bar must emit NaN on that bar — `dropna()`
  is what keeps the gap out of the IC, so a fabricated `±1` would be consumed
  as a real signal. When you cite an IC value, mention the sample size after
  the drop.
- **`pct_change()` does not forward-fill by default** (v0.1.15). A missing close
  becomes a missing return, not `0.0%`. Watch for callers who explicitly pass
  `fill_method="ffill"` — that's the old behaviour and an exception, not the
  default.
...
- **v0.1.15 survivorship-bias caveat**: the bench HTML now renders the
  survivorship-bias flag explicitly. Don't cite IC numbers without checking
  the period coverage in the report footer.

## Reference

- Operator catalogue: `docs/alpha-zoo/spec.md`
- Registry contract: `src/factors/registry.py` (frozen; NaN-mask enforced here)
