---
name: vibe-trading-strategy-discovery
...
- User asks to **create, backtest, or register** a strategy → this is NOT this skill; route to `vibe-trading-strategy-generate` / `vibe-trading-strategy-dev-manager` / `vibe-trading-alpha-zoo`
...
Every run passes five hard gates before it can produce evidence rows — one-to-one with the `vibe-trading-backtest-diagnose` skill's Hard-Gate Checklist, in this order:
