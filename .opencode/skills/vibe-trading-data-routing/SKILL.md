---
name: vibe-trading-data-routing
...
> **v0.1.15 upgrade (2026-09-09)** — added three new sources, two new market
> routings (UK equity, Korea KRX), and promoted Korea/Vietnam to first-class
> engines. Explicit-only sources (`tickerall`, `nobitex`, `wallex`) never join
> an automatic fallback chain — they must be named explicitly in
> `config.json` (`"source": "tickerall"`, etc.).

| Source | Markets | Auth (env key) | Network | Skill |
|--------|---------|----------------|---------|-------|
| tushare | A-shares, funds, macro (no futures — points tier not implemented) | Yes (`TUSHARE_TOKEN`) | China network | tushare |
| akshare | A-shares, US, HK, **China futures (Sina daily endpoints)**, macro, forex | No | Unrestricted | akshare |
| yfinance | US, HK, ETFs, **Canada (.TO/.V)**, **India (.NS/.BO)**, **Vietnam (.VN)**, **UK (.L)** | No | Needs Yahoo access | yfinance |
...
| longbridge | HK / US equities (historical OHLCV) | Yes (`LONGBRIDGE_APP_KEY` + `LONGBRIDGE_APP_SECRET` + `LONGBRIDGE_ACCESS_TOKEN`) | Longbridge SDK | data-routing |
| mt5 | Forex, metals (MetaTrader 5 terminal) | Yes (terminal logged in) | Local terminal | data-routing |
| tickerall | Forex, metals (hosted MT5 via TickerAll HTTP API) — **explicit-only** | Yes (`TICKERALL_API_KEY` + `TICKERALL_ACCOUNT_ID`) | TickerAll API | data-routing |
| pykrx | **Korea (KOSPI .KS / KOSDAQ .KQ)** daily bars (Naver-backed adjusted) | No (`pip install "vibe-trading-ai[krx]"`) | KRX | data-routing |
| india_broker | India NSE/BSE back-fill from live broker (Shoonya / Dhan) | Yes (broker login) | Broker API | data-routing (runner-internal) |
| nobitex | **Crypto IRR (Iran, Toman)** — **explicit-only** | No (keyless public UDF) | nobitex.ir | data-routing |
| wallex | **Crypto IRR (Iran, Toman)** — **explicit-only** | No (keyless public UDF) | wallex.ir | data-routing |
...
| stooq | US equities (daily OHLCV) | No (anti-bot challenge — flagged, not parsed) | Unrestricted | data-routing |
...
| fmp | US equities | Yes (`FMP_API_KEY`) — now serves Stable endpoint, no silent fallback when named explicitly | Unrestricted | data-routing |
...
- **HK stocks**: tencent > eastmoney / yahoo > yfinance > longbridge (key-gated).
- **Canada (TSX .TO / TSXV .V)**: yfinance / yahoo (CAD identity, whole shares).
- **India (NSE .NS / BSE .BO)**: yfinance / yahoo (.NS/.BO) > india_broker (Shoonya/Dhan).
- **Korea (KOSPI .KS / KOSDAQ .KQ)**: pykrx (Naver-adjusted daily, `[krx]` extra) > yfinance > yahoo.
- **Vietnam (HOSE .VN)**: yfinance / yahoo — long-only, ±7% band, 100-share lots.
- **UK (LSE .L / .IL)**: yfinance / yahoo — SDRT 0.5% purchase-side duty.
- **Crypto**: okx (single exchange) > ccxt (multi-exchange). Nobitex / wallex are
  Toman-quoted and **never** join an automatic fallback chain.
- **China Futures**: **`akshare` (Sina daily endpoints, dated + main continuous)
  > `local`** (chain changed in v0.1.15). tushare no longer declares the `futures`
  market because its `pro.fut_daily` sits behind a points tier.
- **Global Futures / Forex / Metals**: yfinance > akshare; **MT5** or
  **`tickerall` (explicit-only, hosted MT5)** when local terminal is unavailable.
...
| US stocks | `TICKER.US` | AAPL.US, MSFT.US, BRK.B.US (dotted class shares supported in v0.1.15) |
| HK stocks | `NNNNN.HK` | 00700.HK, 09988.HK |
| Canada | `TICKER.TO` / `TICKER.V` | TD.TO, PNG.V |
| India | `SCRIP.NS` / `SCRIP.BO` | RELIANCE.NS, 500325.BO |
| Korea | `CODE.KS` / `CODE.KQ` | 005930.KS, 247540.KQ |
| Vietnam | `TICKER.VN` | VIC.VN |
| UK | `TICKER.L` / `TICKER.IL` | SHEL.L, VOD.L |
| Crypto | `SYMBOL-USDT` (spot) | BTC-USDT, ETH-USDT |
| Crypto IRR | `BTCIR-TOMAN` (explicit-source only) | — |
| China Futures | `XXNNNN.EXCHANGE` (dated) or `XX0` (main continuous) | CU2406.SHFE, RB0, IF2512.CFFEX |
| Global Futures | `SYMBOLZ?.EXCHANGE` | CL2412.NYMEX, ESZ4.CME |
| Forex | `XXX/YYY` or `XXXYYY` | USD/CNY, EUR/USD, USDJPY |
...
- **stooq's anti-bot challenge is flagged as a challenge, not parsed as data** —
  the difference between "no data" and "a JavaScript page interpreted as prices"
  (v0.1.15 fix). Treat a stooq "result" as `0 rows` when the challenge banner
  appears in the response.
- **Explicit sources never silently fall back** when named in `config.json`.
  FMP, Tiingo, Yahoo, etc. either serve as themselves or return empty. This is
  intentional — a strategy that names `fmp` should not silently degrade to
  free-tier Yahoo data on a single transient error.
- **Adjustment caliber is stamped on every served frame** (v0.1.15). A row that
  is `split_adjusted`/`dividend_adjusted`/`raw` is recorded next to the price;
  anything unmeasured resolves to `unknown` on purpose (e.g. Yahoo serves
  split-adjusted quotes with no adjustment logic in this repo).
- **Key-gated sources need their env key** (`FINNHUB_API_KEY`,
  `ALPHAVANTAGE_API_KEY`, `TIINGO_API_KEY`, `FMP_API_KEY`, `FRED_API_KEY`,
  `VIBE_TRADING_IWENCAI_KEY`, `TUSHARE_TOKEN`, `LONGBRIDGE_APP_KEY`,
  `TICKERALL_API_KEY`, `TICKERALL_ACCOUNT_ID`). If the key is absent the
  tool/loader is unavailable — route to a free same-market source instead of
  erroring out.
- **Explicit-only sources** (`tickerall`, `nobitex`, `wallex`) never join an
  automatic chain. They must be named explicitly in `config.json` as
  `"source": "tickerall"` etc. The Toman-quoted Iranian sources in particular
  would corrupt a USD-quoted request if they were ever served as a fallback.
