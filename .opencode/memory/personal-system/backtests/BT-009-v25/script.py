...
def load_sector_data(ticker: str) -> pd.DataFrame:
    ...
    try:
        df_etf = load_data(etf_code)
        if df_etf is None or df_etf.empty:
            return pd.DataFrame()
        rename = {'日期': 'date', '股票代码': 'code', '开盘': 'open', '收盘': 'close',
                  '最高': 'high', '最低': 'low', '成交量': 'volume', '成交额': 'amount'}
        df_etf = df_etf.rename(columns={k: v for k, v in rename.items() if k in df_etf.columns})
        if 'date' not in df_etf.columns and df_etf.index.name == 'date':
            df_etf = df_etf.reset_index()
        if 'date' in df_etf.columns:
            df_etf['date'] = pd.to_datetime(df_etf['date'])
            return df_etf.sort_values('date').reset_index(drop=True)
        return df_etf
    except Exception as e:
