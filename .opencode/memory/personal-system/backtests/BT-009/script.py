...
def score_v22_1(signals: dict, w_event: int = 2, w_trend: int = 1) -> tuple:
    """v2.2.1 分级计票 (ml_strategy 已降级到 ×1 趋势位, max=11)"""
    EVENT_SIGNALS = ['candlestick', 'chanlun']
    TREND_SIGNALS = ['alpha_engine_v21', 'technical_basic', 'ichimoku', 'smc',
                     'alpha_zoo', 'multi_factor', 'ml_strategy']
    ...
    max_score = 2 * w_event + 7 * w_trend  # 4 + 7 = 11 (v2.2.1)
...
def compute_at_v22_1(df: pd.DataFrame, cut: int, ticker: str, variant: str = 'v2.2.1',
...
def load_ticker_data(ticker: str, name: str) -> pd.DataFrame:
    """加载日线: tech-pool 优先 → raw_daily → Sina 通用化下载"""
    import glob
    ...
    for f in glob.glob(str(TECH_DIR / f"{ticker}_*.csv")):
        try:
            ...
            pass
    # 2. raw_daily
    raw = DATA_DIR / f"raw_daily_{ticker}.csv"
    if raw.exists():
        try:
            df = pd.read_csv(raw)
            df = df.rename(columns={k: v for k, v in rename.items() if k in df.columns})
            ...
            pass
    # 3. Sina 通用化下载 (load_data 内置)
    print(f"  [{ticker} {name}] 本地无缓存，触发 Sina 下载...")
...
def backtest_ticker_with_position_curve(ticker: str, name: str, start: str, weekday: int,
    ...
    for i in eval_idx:
        ...
        row = compute_at_v22_1(df, cut, ticker, variant, w_event, w_trend)
