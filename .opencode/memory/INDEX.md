...
**最后更新:** **2026-09-15（563530 卫星ETF 换仓 — 方正证券 601901 清出。新增 LAW-005（IPO 密集期 → 板块 forward 6m 跑输）+ HYP-034（中美太空对抗 = 国家级必争之地）；BT-020/021 入库；新增 CONFLICT-LOGIC-012/013 + CONFLICT-USER-AI-TERMINAL；LAW-001 补「样本 <315 根时 WT1 无效」工具边界；`3.2` 预判 4 补 ETF 工具约束。**活跃法则 4 → 5、活跃假设 17 → 18、回测 18 → 21）**
...
| **活跃假设** | **18 个**（**按编号升序排列**，重要度见 Tier 标）：Tier 1 ⭐ = HYP-006/015/016/021/026/029 + **HYP-032（G2双人舞×接力式化债×驯服通胀）** + **HYP-033（G7 长端买家坍缩 → 期限溢价新常态 + 黄金重估）** + **HYP-034（中美太空对抗 = 国家级必争之地，主题层）**；Tier 2 = HYP-002/011/014/017/020/024/028/030；Tier 3 = HYP-027（γ父框架）。另 HYP-025 元评估 |
...
7. **ETF 工具层约束（换仓 563530 实测）**: **A 股 ETF 无 SpaceX 对标物** —— 发射层与 LEO 星座层**全部未上市**。实测："卫星 ETF"**零发射暴露**；"航空航天 ETF"**航空 35-37% > 航天 25%**；"航空航天ETF嘉实"实际跟踪中证高端装备细分 50。发射配套最高 = **159206（~6.5%）**；唯一含火箭 = **563380（中天火箭 0.78%）**。→ **赛道判断与实际工具之间存在系统性缺口**（属 HYP-034 的"可投资性约束"）
...
- **数据缺口**: (1) 8 只公募基金净值 5 周未刷新（Morningstar 不能识别 A 股 6 位基金代码）；(2) 563530 建仓金额/份额未报告 → 无法按 49% 年化波动做仓位上限；(3) 中证 931594 / 国证商用卫星通信 指数的**编制方案文件未取到** → HYP-034 的"发射暴露"分类是业务归类而非指数规则。
