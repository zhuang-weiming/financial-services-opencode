...
class WebSocketChannel(BaseChannel):
    ...
    async def _dispatch_http(self, connection: Any, request: WsRequest) -> Any:
        ...
        token_path = _normalize_config_path(self.config.token_issue_path) if self.config.token_issue_path else ""
        if token_path and got == token_path:
    ...
    def _issue_token_response(self, connection: Any, request: WsRequest) -> Any:
        """Issue a short-lived WebSocket token over the configured HTTP path."""
        ...
        return connection.respond(
            200,
            json.dumps({"token": token, "expires_in": expires_in}, ensure_ascii=False),
        )
