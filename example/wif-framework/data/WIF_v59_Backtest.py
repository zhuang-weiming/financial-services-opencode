...
PACKAGE_ROOT = REPO_ROOT / '.opencode' / 'python' / 'wif-framework'
if str(PACKAGE_ROOT) not in sys.path:
    sys.path.insert(0, str(PACKAGE_ROOT))

from wif_framework.phase import phase1_status as package_phase1_status
from wif_framework.portfolio import PortfolioLedger, select_rebalance_assets


def _latest_path(paths, label):
    candidates = sorted(paths)
    if not candidates:
        raise FileNotFoundError(f"No {label} found under {SCRIPT_DIR}")
    return candidates[-1]


MERGED_PATH = _latest_path(SCRIPT_DIR.glob('_merged_prices_*.csv'), '_merged_prices_*.csv')
TICKER_DIR = _latest_path((p for p in SCRIPT_DIR.glob('tickers_*') if p.is_dir()), 'tickers_* directory')
_tag_match = re.search(r'(\d{8})$', MERGED_PATH.stem)
SNAPSHOT_TAG = _tag_match.group(1) if _tag_match else datetime.now().strftime('%Y%m%d')
OUT_DIR = SCRIPT_DIR
DATA_DIR = SCRIPT_DIR
REPORT_DIR = SCRIPT_DIR


def resolve_ticker_file(prefix):
    matches = sorted(TICKER_DIR.glob(f'{prefix}_*.csv'))
    if matches:
        return matches[-1]
    direct = TICKER_DIR / f'{prefix}.csv'
    if direct.exists():
        return direct
    raise FileNotFoundError(f"No data file for {prefix} in {TICKER_DIR}")

# ─────────────────────────────────────────────────────────
# ① 加载数据
# ─────────────────────────────────────────────────────────
print("① 加载合并价格数据...")
prices = pd.read_csv(MERGED_PATH, parse_dates=['Date'], index_col='Date')
...
print("  加载真实BAA信用利差数据...")
cred = pd.read_csv(resolve_ticker_file('CreditSpread_BAA'), parse_dates=['Date'], index_col='Date')
...
# ── 固收代理：优先使用 SHV 真实数据
try:
    shv_path = resolve_ticker_file('SHV')
...
for short_name, ticker in factor_etfs.items():
    try:
        path = resolve_ticker_file(ticker)
    except FileNotFoundError:
        print(f"  {short_name:6s}: 数据未找到，跳过")
        continue
...
# ── Hybrid Phase1（v5.9）──
# Soft CSI entry requires three consecutive days; recovery requires five days.
# F33b and F29 are independent immediate hard triggers, including when CSI > 2.
prices['F33b_EMERGENCY_override'] = prices['F33b_trigger'].astype(int)
prices['F29_hard_trigger'] = (prices['F29_bp'] > 500).astype(int)
prices['F29_EMERGENCY_override'] = prices['F29_hard_trigger'].astype(int)
prices['Phase1_status'] = package_phase1_status(
    prices,
    csi=prices['CSI'],
...
print(f"  F33b硬触发：{f33b_stats} 天 | 独立立即进入EMERGENCY：{override_stats} 天")
f29_stats = prices['F29_hard_trigger'].sum()
f29_override_stats = prices['F29_EMERGENCY_override'].sum()
print(f"  ★ F29>500bp 硬触发：{f29_stats} 天 | 独立立即进入EMERGENCY：{f29_override_stats} 天")
...
dgs10   = load_raw_etf(resolve_ticker_file('DGS10'),  'DGS10')
t10yie  = load_raw_etf(resolve_ticker_file('T10YIE'), 'T10YIE')
...
prices['MCI_adjusted']  = (prices['MCI_layer2'] + prices['MCI_layer3']).fillna(0)
...
def calc_v68b(phase1, mci, quad, t10yie=2.0, cpi=2.5):
    ...
    if eq + alt > 0.97 and alt > 0:
        sc = max(0.0, 0.97-eq) / alt
        gld*=sc; xle*=sc; pdbc*=sc; alt=gld+xle+pdbc
    fi = max(0.0, 1.0-eq-alt)
    raw = {'stock':max(0,eq),'fi':fi,'gld':max(0,gld),'xle':max(0,xle),'pdbc':max(0,pdbc)}
    tot = sum(raw.values())
    result = {k: round(v/tot,4) for k,v in raw.items()}
    residual_key = max(result, key=result.get)
    result[residual_key] = round(result[residual_key] + (1.0-sum(result.values())), 4)
    return result
...
for idx, row in prices.iterrows():
    ...
    if phase1 == 'EMERGENCY':
        ...
        if total > 0:
            t67_norm = {k: v/total for k, v in t67.items()}
            # Ensure exact 1.0 sum — rounding to 4dp introduces drift that
            # PortfolioLedger.rebalance's 1e-8 tolerance will reject.
            residual = 1.0 - sum(t67_norm.values())
            key = max(t67_norm, key=t67_norm.get)
            t67_norm[key] = t67_norm[key] + residual
            t67 = t67_norm
...
def fee_rate_for_fill(ticker, action, is_initial):
    if action == 'SELL':
        return REDEMPTION_FEE
    return get_initial_fee_rate(ticker) if is_initial else ADD_SWITCH_FEE


def normalized_asset_returns(row):
    result = {}
    for ticker in set(target_v59.get(row.name, {})) | set(target_v68b.get(row.name, {})):
        value = row.get(f'{ticker}_ret', 0.0)
        result[ticker] = 0.0 if pd.isna(value) else float(value)
    return result
...
for i, (idx, row) in enumerate(prices.iterrows()):
    ...
    # Phase1 already contains 3-day soft-entry and 5-day recovery hysteresis.
    # A confirmed state transition therefore triggers execution immediately.
    if prev_phase1 is not None and curr_phase1 != prev_phase1:
    ...
    def get_category_key(ticker):
        """将ETF映射到资产类别"""
        if ticker in ('VTI', 'QQQ'):
            return 'VTI_QQQ'
        elif ticker in EQUITY_FUND_TYPES:
            return 'EQUITY_FUND'
...
print("  非触发日不成交；实际持仓继续漂移并控制后续P&L")

ret_v59, ret_v68b, ret_spy = [], [], []
ret_v59_net, ret_v68b_net = [], []
log_v59, log_v68b = [], []

ledger59 = PortfolioLedger(1.0)
ledger68b = PortfolioLedger(1.0)
gross_ledger59 = PortfolioLedger(1.0)
gross_ledger68b = PortfolioLedger(1.0)

nav59_seq, nav68b_seq = [], []
nav59_gross_seq, nav68b_gross_seq = [], []
daily_fee_usd59, daily_fee_usd68b = [], []
rebalance_event59, rebalance_event68b = [], []


def execute_rebalance(net_ledger, gross_ledger, target, row, row_number, is_initial, force):
    current = net_ledger.weights()
    panic_multiplier = 2.0 if float(row['VIX']) > 40 else 1.0
    bands = {
        ticker: calc_dynamic_band(ticker, float(row['VIX']), row_number) * panic_multiplier
        for ticker in set(current) | set(target)
    }
    selected = select_rebalance_assets(current, target, bands, force=(force or is_initial))
    fx_rate = get_fx_rate(row.name)
    minimum_rmb = MIN_INITIAL_RMB if is_initial else MIN_ADD_SWITCH_RMB
    result = net_ledger.rebalance(
        target,
        selected_assets=selected,
        fee_rate=lambda ticker, action: fee_rate_for_fill(ticker, action, is_initial),
        minimum_trade_value=minimum_rmb,
        capital_scale=INITIAL_NAV_USD * fx_rate,
    )

    filled_assets = {fill.ticker for fill in result.fills}
    if filled_assets:
        gross_ledger.rebalance(target, selected_assets=filled_assets)

    trades = {}
    for fill in result.fills:
        trades[fill.ticker] = {
            'action': fill.action,
            'weight_change': fill.weight_change,
            'amount_usd': fill.notional * INITIAL_NAV_USD,
            'amount_rmb': fill.notional * INITIAL_NAV_USD * fx_rate,
            'fee_rate': fill.fee_rate,
            'fee_usd': fill.fee * INITIAL_NAV_USD,
            'pre_weight': fill.pre_weight,
...
for i, (idx, row) in enumerate(prices.iterrows()):
    ...
    previous_nav59 = ledger59.nav
    previous_nav68b = ledger68b.nav
    previous_gross59 = gross_ledger59.nav
    previous_gross68b = gross_ledger68b.nav

    asset_returns = normalized_asset_returns(row)
    ledger59.apply_returns(asset_returns)
    ledger68b.apply_returns(asset_returns)
    gross_ledger59.apply_returns(asset_returns)
    gross_ledger68b.apply_returns(asset_returns)
    ...
    force = is_factor_sw or is_state_chg or is_new_cat
    ...
    if any_trigger or is_initial:
        result59, trades59 = execute_rebalance(
            ledger59, gross_ledger59, t58, row, i, is_initial, force
        )
        if result59.fills:
            fee_usd59 = result59.total_fee * INITIAL_NAV_USD
            event59 = 1
            log_v59.append({
                'date': idx, 'phase1': phase1, 'quad': quad, 'mci': mci,
                'signal': sig, 'best_factor': best_f,
                'trades': trades59, 'total_fee_usd': fee_usd59,
                'nav_before': result59.nav_before, 'nav_after': result59.nav_after,
            })

        result68b, trades68b = execute_rebalance(
            ledger68b, gross_ledger68b, t67, row, i, is_initial, force
        )
        if result68b.fills:
            fee_usd68b = result68b.total_fee * INITIAL_NAV_USD
            event68b = 1
            log_v68b.append({
                'date': idx, 'phase1': phase1, 'quad': quad, 'mci': mci,
                'signal': sig, 'best_factor': best_f,
                'trades': trades68b, 'total_fee_usd': fee_usd68b,
                'nav_before': result68b.nav_before, 'nav_after': result68b.nav_after,
            })

    nav59_seq.append(ledger59.nav)
    nav68b_seq.append(ledger68b.nav)
    nav59_gross_seq.append(gross_ledger59.nav)
    nav68b_gross_seq.append(gross_ledger68b.nav)
    daily_fee_usd59.append(fee_usd59)
    daily_fee_usd68b.append(fee_usd68b)
    rebalance_event59.append(event59)
    rebalance_event68b.append(event68b)

    ret_v59_net.append(ledger59.nav / previous_nav59 - 1.0)
    ret_v68b_net.append(ledger68b.nav / previous_nav68b - 1.0)
    ret_v59.append(gross_ledger59.nav / previous_gross59 - 1.0)
    ret_v68b.append(gross_ledger68b.nav / previous_gross68b - 1.0)

    if i % 500 == 0:
        print(f"DEBUG: i={i}, nav59={ledger59.nav:.6f}, nav68b={ledger68b.nav:.6f}")
...
prices['ret_v59_net'] = ret_v59_net
prices['ret_v68b_net'] = ret_v68b_net
prices['ret_v59_gross'] = ret_v59
prices['ret_v68b_gross'] = ret_v68b

prices['v59_cum_gross'] = pd.Series(nav59_gross_seq, index=prices.index)
prices['v68b_cum_gross'] = pd.Series(nav68b_gross_seq, index=prices.index)
prices['v59_cum_net'] = pd.Series(nav59_seq, index=prices.index)
prices['v68b_cum_net'] = pd.Series(nav68b_seq, index=prices.index)
prices['v59_cum'] = prices['v59_cum_gross']
prices['v68b_cum'] = prices['v68b_cum_gross']
prices['spy_cum'] = (1 + pd.Series(ret_spy, index=prices.index).fillna(0)).cumprod()

first_post_fee_nav59 = float(nav59_seq[0])
first_post_fee_nav68b = float(nav68b_seq[0])
prices['v59_operating_cum'] = prices['v59_cum_net'] / first_post_fee_nav59
prices['v68b_operating_cum'] = prices['v68b_cum_net'] / first_post_fee_nav68b
prices['fee_usd_v59'] = daily_fee_usd59
prices['fee_usd_v68b'] = daily_fee_usd68b
prices['rebalance_event_v59'] = rebalance_event59
prices['rebalance_event_v68b'] = rebalance_event68b

total_cost_display59 = ledger59.total_fees * INITIAL_NAV_USD
total_cost_display68b = ledger68b.total_fees * INITIAL_NAV_USD
n59 = len(log_v59)
n68b = len(log_v68b)
fills59 = sum(len(entry['trades']) for entry in log_v59)
fills68b = sum(len(entry['trades']) for entry in log_v68b)

print(f"  v5.9 再平衡事件: {n59} 次 | 资产级成交: {fills59} 笔 | 累计成本: ${total_cost_display59:,.2f}")
print(f"  v6.8.1 再平衡事件: {n68b} 次 | 资产级成交: {fills68b} 笔 | 累计成本: ${total_cost_display68b:,.2f}")
...
def ps(df, col, label, nav_col=None, initial_capital=1.0):
    if nav_col and nav_col in df.columns:
        nav = df[nav_col].dropna()
        nav_with_initial = pd.concat([
            pd.Series([initial_capital], index=[nav.index[0] - pd.Timedelta(nanoseconds=1)]),
            nav,
        ])
        capital_return = float(nav.iloc[-1] / initial_capital - 1.0)
        operating_return = float(nav.iloc[-1] / nav.iloc[0] - 1.0)
        initial_fee = float(initial_capital - nav.iloc[0])
        dd = maxdd(nav_with_initial)
    else:
        nav = (1+df[col].fillna(0)).cumprod() * initial_capital
        capital_return = float(nav.iloc[-1] / initial_capital - 1.0)
        operating_return = capital_return
        initial_fee = 0.0
        dd = maxdd(nav)
    ...
    return {
        'label': label,
        'cumret': capital_return,
        'operating_return': operating_return,
...
s59_net=ps(ev,'ret_v59_net','v5.9_net',nav_col='v59_cum_net')
s68b_net=ps(ev,'ret_v68b_net','v6.8.1_net',nav_col='v68b_cum_net')
ss=ps(ev,'ret_spy','SPY')
ev_g = prices.dropna(subset=['ret_v59_gross','ret_v68b_gross'])
s59_gross=ps(ev_g,'ret_v59_gross','v5.9_gross',nav_col='v59_cum_gross')
s68b_gross=ps(ev_g,'ret_v68b_gross','v6.8.1_gross',nav_col='v68b_cum_gross')

print(f"\n{'='*72}")
print(f"{'指标':<18} {'v5.9(有摩擦)':>14} {'v5.9(无摩擦)':>14} {'v6.8.1(有摩擦)':>14} {'v6.8.1(无摩擦)':>14} {'SPY':>10}")
print("-"*110)
print(f"{'资本口径累计收益':.<18} {s59_net['cumret']:>+14.1%} {s59_gross['cumret']:>+14.1%} {s68b_net['cumret']:>+14.1%} {s68b_gross['cumret']:>+14.1%} {ss['cumret']:>+10.1%}")
print(f"{'费后运营收益':.<18} {s59_net['operating_return']:>+14.1%} {'—':>14} {s68b_net['operating_return']:>+14.1%} {'—':>14} {'—':>10}")
print(f"{'初始费用/NAV':.<18} {s59_net['initial_fee']:>14.2%} {'0.00%':>14} {s68b_net['initial_fee']:>14.2%} {'0.00%':>14} {'0.00%':>10}")
...
print(f"{'再平衡事件':.<18} {n59:>14}次 {'—':>14} {n68b:>14}次 {'—':>14} {'—':>10}")
print(f"{'资产级成交':.<18} {fills59:>14}笔 {'—':>14} {fills68b:>14}笔 {'—':>14} {'—':>10}")
print(f"{'累计成本(USD)':.<18} ${total_cost_display59:>13,.0f} {'—':>14} ${total_cost_display68b:>13,.0f} {'—':>14}")
print("-"*110)
...
<div class="container">
    ...
    <div class="section">
        ...
        <div style="background:#fff; border-radius:8px; padding:18px; box-shadow:0 1px 4px rgba(0,0,0,0.06);">
            ...
            <div style="display:flex; gap:18px; margin-top:10px; font-size:0.78rem; color:#666; flex-wrap:wrap; justify-content:center;">
                ...
                <span>📊 数据源: WIF_v59_nav_{SNAPSHOT_TAG}.csv</span>
    ...
    <div class="section">
        ...
        <table>
            ...
            <tr>
                <td>费后运营收益（首个扣费后NAV起算）</td>
                <td>{s59_net['operating_return']:+.1%}</td><td>—</td>
                <td>{s68b_net['operating_return']:+.1%}</td><td>—</td><td>—</td>
            </tr>
            <tr>
                <td>初始费用/NAV</td>
                <td>{s59_net['initial_fee']:.2%}</td><td>0.00%</td>
                <td>{s68b_net['initial_fee']:.2%}</td><td>0.00%</td><td>0.00%</td>
            </tr>
            <tr>
                <td>年化收益</td>
            ...
            <tr>
                <td>资产级成交（fills）</td>
                <td colspan="2">{fills59} 笔</td>
                <td colspan="2">{fills68b} 笔</td>
                <td>—</td>
            </tr>
            <tr>
                <td>累计成本损耗</td>
        ...
        <p style="font-size:12px; color:#777; margin-top:10px; line-height:1.7;">
            资本口径累计收益 = 最终净NAV / 1.0 - 1，完整计入初始费用；费后运营收益 = 最终净NAV / 首个扣费后NAV - 1，仅作为补充口径。<br>
            再平衡事件按发生实际成交的日期计数；资产级成交按每个ticker的实际fill计数。SKIP和未触发交易不改变持仓或P&amp;L。<br>
            * v6.8.1含存活者偏差；历史回测仅供研究，不构成未来收益预测。
        </p>
    </div>
    ...
    <div class="section">
        ...
        <table>
            <tr>
                <th>年份</th>
                <th><span class="tag-v59">v5.9</span> 再平衡事件</th>
    ...
    <div class="section">
        <div class="section-header">
            <h2 class="section-title" style="margin:0; border:none; padding:0;">
                <span class="tag-v59">v5.9</span> 成交日志（{n59} 个再平衡事件 / {fills59} 笔资产级成交）
            </h2>
    ...
    <div class="section">
        <div class="section-header">
            <h2 class="section-title" style="margin:0; border:none; padding:0;">
                <span class="tag-v68b">v6.8.1</span> 成交日志（{n68b} 个再平衡事件 / {fills68b} 笔资产级成交）
            </h2>
    ...
    <div class="section">
        ...
        <table>
            ...
            <tr><td><strong>再平衡事件 / 资产级成交</strong></td><td>{n59}次 / {fills59}笔</td><td>{n68b}次 / {fills68b}笔</td></tr>
        </table>
    </div>
...
report_path = REPORT_DIR / f'WIF_v59_Report_{SNAPSHOT_TAG}.html'
with open(report_path, 'w', encoding='utf-8') as f:
    f.write(html_content)
print(f"  ✅ 对比报告已保存: {report_path}")

# 保存净值
nav_path = REPORT_DIR / f'WIF_v59_nav_{SNAPSHOT_TAG}.csv'
...
trade_csv_path = REPORT_DIR / f'WIF_v59_trade_log_{SNAPSHOT_TAG}_HSCBC.csv'
